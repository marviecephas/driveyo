...
function startTelemetryTrip(driverObj, mode = 'normal') {
  selectedDriverObj = driverObj;
...
function restartTelemetrySimulation() {
  if (selectedDriverObj) {
    const dbIndex = drivers.findIndex(d => d.license === selectedDriverObj.license);
    if (dbIndex !== -1) {
      drivers[dbIndex].checkedIn = true;
      drivers[dbIndex].pinStatus = "verified";
    }
    selectedDriverObj.checkedIn = true;
    selectedDriverObj.pinStatus = "verified";
    ...
    startTelemetryTrip(selectedDriverObj, 'normal');
  } else {
    startTelemetryTrip(drivers[0], 'normal');
  }
}
